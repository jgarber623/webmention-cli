module WebmentionCLI
  class CLI < Thor
    ...
    def endpoint(url)
      ...
      raise Thor::MalformattedArgumentError, "ERROR: Expected '#{url}' to be an absolute URL (e.g. https://example.com)"
    end
    ...
    def verify(source, target)
      webmention_verified = Webmention::Verification.client(source, target, strict: options[:strict]).verified?
      ...
      raise Thor::MalformattedArgumentError, "ERROR: Expected '#{source}' and '#{target}' to be absolute URLs (e.g. https://example.com)"
    end
  end
end
